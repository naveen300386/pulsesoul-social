...
4. Happy? Run it again with **Preview only** unticked. Check your accounts.
5. From then on it runs by itself, twice a day, around **09:07 and 20:07 IST**.

**About those times:** GitHub's free scheduler is shared. Runs are commonly
5–30 minutes late, and occasionally one is skipped entirely — GitHub does not
retry a missed run. Nothing is lost when that happens; the queue simply
continues from where it was on the next run.

To change the times, edit the two `cron:` lines in
`.github/workflows/autopost.yml`. They are in UTC — subtract 5 hours 30
minutes from the IST time you want, and avoid `:00` and `:30`.
